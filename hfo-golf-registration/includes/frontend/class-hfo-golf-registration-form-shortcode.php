...
class HFO_Golf_Registration_Form_Shortcode {
	...
	public function render_shortcode( $atts ) {
		...
		?>
		<form class="hfo-golf-registration-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-hfo-golf-registration-form data-recaptcha-site-key="<?php echo esc_attr( $recaptcha_site_key ); ?>" data-recaptcha-action="<?php echo esc_attr( self::RECAPTCHA_ACTION ); ?>" data-golf-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'golf_price' ) ); ?>" data-lunch-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'lunch_price' ) ); ?>" data-dinner-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'dinner_price' ) ); ?>" data-platinum-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'platinum_sponsor_price' ) ); ?>" data-gold-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'gold_sponsor_price' ) ); ?>" data-silver-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'silver_sponsor_price' ) ); ?>" data-tee-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'tee_sponsor_price' ) ); ?>" novalidate>
			...
			<section class="hfo-golf-registration-step" data-hfo-golf-registration-step data-step-key="main_contact" hidden>
				...
				<?php $this->render_state_field( 'main_contact_state', esc_html__( 'State', 'hfo-golf-registration' ) ); ?>
			...
			<section class="hfo-golf-registration-step" data-hfo-golf-registration-step data-step-key="sponsorship" hidden>
				...
				<div data-hfo-golf-sponsor-fields hidden>
					...
					<?php $this->render_state_field( 'sponsor_state', esc_html__( 'Sponsor State', 'hfo-golf-registration' ) ); ?>
			...
			<section class="hfo-golf-registration-step" data-hfo-golf-registration-step data-step-key="review" hidden>
				...
				<dl class="hfo-golf-registration-review" aria-live="polite">
					...
					<dt><?php esc_html_e( 'Additional Lunch Quantity', 'hfo-golf-registration' ); ?></dt><dd data-summary="additional_lunch_qty">0</dd>
					<dt><?php esc_html_e( 'Additional Dinner Quantity', 'hfo-golf-registration' ); ?></dt><dd data-summary="additional_dinner_qty">0</dd>
	...
	public function handle_submission() {
		...
		if ( 'additional_guests' === $meta['registration_type'] ) {
			$this->validate_additional_guests_only_submission( $meta );
		}
	...
	/**
	 * Validates fields that are required for Additional Guests-only registrations.
	 *
	 * @param array<string,string> $meta Sanitized submitted meta.
	 * @return void
	 */
	private function validate_additional_guests_only_submission( $meta ) {
		foreach ( array( 'main_contact_name', 'main_contact_email', 'main_contact_phone', 'main_contact_address', 'main_contact_city', 'main_contact_state', 'main_contact_zip' ) as $field_key ) {
			if ( '' === trim( (string) $meta[ $field_key ] ) ) {
	...
	private function calculate_quantities_and_totals( $event_id, $meta ) {
		...
		$player_lunch_qty      = 0;
		$player_dinner_qty     = 0;
		$additional_lunch_qty  = absint( $meta['additional_lunch_count'] );
		$additional_dinner_qty = absint( $meta['additional_dinner_count'] );

		if ( 'team' === $meta['registration_type'] ) {
			$golf_qty = 4;
		} elseif ( 'individual' === $meta['registration_type'] ) {
			$golf_qty = 1;
		}
		...
		$subtotal = ( $golf_qty * $this->get_event_price( $event_id, 'golf_price' ) )
			+ ( $additional_lunch_qty * $this->get_event_price( $event_id, 'lunch_price' ) )
			+ ( $additional_dinner_qty * $this->get_event_price( $event_id, 'dinner_price' ) )
		...
		return array_merge(
			array(
				...
				'additional_lunch_qty'  => (string) $additional_lunch_qty,
				'additional_dinner_qty' => (string) $additional_dinner_qty,
				'lunch_qty'             => (string) $additional_lunch_qty,
				'dinner_qty'            => (string) $additional_dinner_qty,
	...
	/**
	 * Gets WooCommerce-compatible US state options.
	 *
	 * @return array<string,string>
	 */
	...
	/**
	 * Sanitizes a posted state field to a valid US state code.
	 *
	 * @param string $key Posted key.
	 * @return string
	 */
	...
	/**
	 * Normalizes and validates a US state value.
	 *
	 * @param string $value Submitted state value.
	 * @return string
	 */
	private function sanitize_state_code( $value ) {
		$state = strtoupper( trim( sanitize_text_field( (string) $value ) ) );
		...
		if ( isset( $state_name_map[ $state ] ) ) {
			$state = $state_name_map[ $state ];
		}

		return array_key_exists( $state, $this->get_us_state_options() ) ? $state : '';
	}
	...
	private function render_participant_fields( $prefix, $legend ) {
		...
		<fieldset class="hfo-golf-registration-fieldset">
			...
			<?php $this->render_state_field( $prefix . '_state', esc_html__( 'State', 'hfo-golf-registration' ) ); ?>
	...
	/**
	 * Renders a US state select field.
	 *
	 ...
	 * @return void
	 */
	private function render_state_field( $name, $label, $required = false ) {
		$options = array( '' => __( 'Select a state', 'hfo-golf-registration' ) ) + $this->get_us_state_options();
		$this->render_select_field( $name, $label, $options, $required );
	}
