...
class HFO_Golf_Registration_Form_Shortcode {
		...
		<form class="hfo-golf-registration-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-hfo-golf-registration-form data-golf-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'golf_price' ) ); ?>" data-lunch-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'lunch_price' ) ); ?>" data-dinner-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'dinner_price' ) ); ?>" data-platinum-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'platinum_sponsor_price' ) ); ?>" data-gold-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'gold_sponsor_price' ) ); ?>" data-silver-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'silver_sponsor_price' ) ); ?>" data-tee-sponsor-price="<?php echo esc_attr( $this->get_event_price( $event_id, 'tee_sponsor_price' ) ); ?>">
			...
			<section class="hfo-golf-registration-step" data-hfo-golf-registration-step data-step-key="registration_type">
				...
				$this->render_select_field(
					...
					array(
						...
						'additional_guests' => esc_html__( 'Additional Guests Only', 'hfo-golf-registration' ),
					),
					true
				);
				?>
			...
			<section class="hfo-golf-registration-step" data-hfo-golf-registration-step data-step-key="review" hidden>
				...
				<dl class="hfo-golf-registration-review" aria-live="polite">
					...
					<dt><?php esc_html_e( 'Player Lunch Attendance', 'hfo-golf-registration' ); ?></dt><dd data-summary="player_lunch_attendance">0</dd>
					<dt><?php esc_html_e( 'Player Dinner Attendance', 'hfo-golf-registration' ); ?></dt><dd data-summary="player_dinner_attendance">0</dd>
					<dt><?php esc_html_e( 'Additional Lunch Guests', 'hfo-golf-registration' ); ?></dt><dd data-summary="lunch_qty">0</dd>
					<dt><?php esc_html_e( 'Additional Dinner Guests', 'hfo-golf-registration' ); ?></dt><dd data-summary="dinner_qty">0</dd>
	...
	public function handle_submission() {
		...
		if ( 'additional_guests' === $meta['registration_type'] && ! $this->has_valid_additional_guests_submission( $meta ) ) {
			wp_die( esc_html__( 'Please provide main contact details, guest details, and at least one lunch or dinner guest.', 'hfo-golf-registration' ) );
		}
	...
	private function calculate_quantities_and_totals( $event_id, $meta ) {
		...
		$player_lunch_attendance  = 0;
		$player_dinner_attendance = 0;
		...
		return array_merge(
			array(
				...
				'player_lunch_attendance'  => (string) $player_lunch_attendance,
				'player_dinner_attendance' => (string) $player_dinner_attendance,
	...
	/**
	 * Checks whether Additional Guests Only has the required contact and guest details.
	 *
	 * @param array<string,string> $meta Sanitized submitted meta.
	 * @return bool
	 */
	private function has_valid_additional_guests_submission( $meta ) {
		return '' !== trim( $meta['main_contact_name'] )
			&& is_email( $meta['main_contact_email'] )
			&& '' !== trim( $meta['additional_guests_details'] )
			&& ( absint( $meta['additional_lunch_count'] ) + absint( $meta['additional_dinner_count'] ) ) > 0;
	}
	...
	private function render_participant_fields( $prefix, $legend ) {
		...
		<fieldset class="hfo-golf-registration-fieldset">
			...
			<?php $this->render_hidden_checkbox_field( $prefix . '_golf_selected', true ); ?>
	...
	/**
	 * Renders an internal hidden checkbox value.
	 *
	 * @param string $name    Field name.
	 * @param bool   $checked Whether the hidden checkbox value is selected.
	 * @return void
	 */
	private function render_hidden_checkbox_field( $name, $checked = false ) {
		?>
		<input id="<?php echo esc_attr( $name ); ?>" name="<?php echo esc_attr( $name ); ?>" type="hidden" value="<?php echo $checked ? '1' : '0'; ?>" />
		<?php
	}
