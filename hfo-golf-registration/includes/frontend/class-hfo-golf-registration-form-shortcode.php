...
class HFO_Golf_Registration_Form_Shortcode {
	...
	public function handle_submission() {
		...
		if ( 'additional_guests' === $meta['registration_type'] ) {
			$this->validate_additional_guests_submission( $meta );
		} elseif ( 'sponsor_only' !== $meta['registration_type'] && ! is_email( $meta['main_contact_email'] ) ) {
	...
	/**
	 * Validates additional guests-only registrations.
	 *
	 * @param array<string,string> $meta Sanitized submitted meta.
	 * @return void
	 */
	private function validate_additional_guests_submission( $meta ) {
		...
		if ( '' === $meta['additional_guests_details'] ) {
			wp_die( esc_html__( 'Please enter additional guests details.', 'hfo-golf-registration' ) );
		}
	...
	private function get_sanitized_submission_meta( $event_id ) {
		$registration_type    = $this->sanitize_choice( 'registration_type', array( 'team', 'individual', 'additional_guests', 'sponsor_only' ), 'individual' );
